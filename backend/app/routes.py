...
# *------------GAME RELATED ROUTES------------*
@app.route('/game/<game_id>/config', methods=['GET'])
def get_game_config(game_id):
    ...
    return jsonify({'success': True, 'config': game.config_dict()}), 200
...
def get_questions(game_id):
    ...
    questions = json.loads(game_data.get('questions'))
    answers = json.loads(game_data.get('answers'))
    return jsonify({'success': True, 'questions': questions, 'answers': answers}), 200
...
def update_player(game_id, username):
    ...
    db.session.commit()
    return jsonify({'success': True, 'msg': 'Player updated'}), 200
